...
def count_histograms(alg_ds):
    ...
    zero_percentages = np.array([(np.count_nonzero(mask == 255) / mask.size) * 100 for _, mask in alg_ds])
    perc_ranges = np.arange(0, 101, 1)
    hist, bin_edges = np.histogram(zero_percentages, bins=perc_ranges)

    # plt.bar(bin_edges[:-1], hist / len(alg_ds), width=8)
    plt.hist(zero_percentages, weights=np.ones(len(zero_percentages)) / len(zero_percentages), bins=range(0,100,2))
